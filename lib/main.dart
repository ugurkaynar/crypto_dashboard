...
// ==========================================
// 1. MODEL KATMANI (Veri Doğruluğu & Yapı)
// ==========================================
class CryptoData {
...
List<CryptoData> _parseKlinesBackground(String responseBody) {
  final decoded = json.decode(responseBody);
  if (decoded is List) {
    return decoded.map((item) => CryptoData.fromJson(item)).toList();
  } else {
...
// ==========================================
// 2. SERVİS KATMANI (Binance API Entegrasyonu)
// ==========================================
class BinanceService {
  static const String _baseUrl = 'https://api.binance.com/api/v3';

  Future<List<CryptoData>> fetchKlines(String symbol, {String interval = '1d', int limit = 500}) async {
  ...
  Future<TickerData> fetchTicker24h(String symbol) async {
...
// ==========================================
// 3. STATE MANAGEMENT (ViewModel Katmanı)
// ==========================================
  ...
  Future<void> loadData(String symbol, {String? interval}) async {
    ...
    try {
      final results = await Future.wait([
        _service.fetchKlines(symbol.trim(), interval: currentTimeframe),
        _service.fetchTicker24h(symbol.trim()),
      ]);
      
      klines = results[0] as List<CryptoData>;
...
// ==========================================
// 5. UI (Kullanıcı Arayüzü) Katmanı
// ==========================================
...
// ==========================================
// 6. PİYASA EKRANI (Market Screen - KeepAlive Uyumlu)
// ==========================================
...
class _MarketScreenState extends State<MarketScreen> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildChart() {
    ...
    final List<Candle> candles = widget.viewModel.klines.reversed.map((e) => Candle(
      date: e.timestamp, open: e.open, high: e.high, low: e.low, close: e.close, volume: e.volume,
    )).toList();
